		...
		{			
			...
			var divAtomo = $('<div>').attr({'id': _id + '-' + cont,
									...
									.css({'top': posicao.top + 'px',
											...
											'background': 'url("imgs/atomos/' + _id +'.png")'})
									.hover(function(){tileEmQueMouseEstá = this.id;}, function(){tileEmQueMouseEstá = "";})
			...
			if(estaNumerado && elementosNoJogo[i].nome == 'carbono') {
				$(elementosNoJogo[i].div).css({'background-image': 'url("imgs/atomos/C_preto.png")'});
...
function desenharLigacao(atomo1, atomo2, lig) {
	...
	var image = $('<div>')
				...
				.css({
					...
					'background-image': 'url("imgs/atomos/' + ligacaoAtual + '.png")',
				})
				.css({
...
function numerarCarbonos() {
	if(!estaNumerado){
		...
		for(var i = 0; i< elementosNoJogo.length; i++) {
			if(elementosNoJogo[i].nome == 'carbono') {
				$(elementosNoJogo[i].div).css({'background-image': 'url("imgs/atomos/carbono.png")'});
			}
		}
		estaNumerado = false;
	}
